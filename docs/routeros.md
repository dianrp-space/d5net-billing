...
```
{portal_base_url}/isolir/{tenantSlug}
```

Contoh: `https://billing.example.com/isolir/acme`
...
4. HTTP dari pool isolir masuk **Web Proxy** → `redirect-to` ke URL isolir di atas.
...
3. **`/ip/proxy`** + access allow/deny+redirect-to URL isolir  
...
/ppp profile add name=isolir remote-address=isolir rate-limit=1M/1M
...
add src-address=10.250.0.0/24 action=deny \
  redirect-to="https://billing.example.com/isolir/acme" comment=drp-isolir:proxy-redirect
...
add chain=forward src-address=10.250.0.0/24 dst-address=billing.example.com action=accept comment=drp-isolir:portal
```

Catatan: Web Proxy hanya mengintercept HTTP (port 80). HTTPS ke host billing harus di-allow di filter supaya halaman isolir/login bisa load.
