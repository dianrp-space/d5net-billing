...
func TestFormatInvoiceNumber(t *testing.T) {
	...
	got := formatInvoiceNumber("BTC-2026090001", at, "AB3K7Z")
	if got != "INV-BTC-2026090001-092026-AB3K7Z" {
		t.Fatalf("got %q", got)
	}
}
...
func TestFormatInvoiceNumberSanitizes(t *testing.T) {
	...
	if got != "INV-PLG-01A-012026-XXXXXX" {
		t.Fatalf("got %q", got)
	}
}
...
func TestFormatInvoiceNumberMaxLen(t *testing.T) {
	...
	if !strings.HasPrefix(got, "INV-") || !strings.HasSuffix(got, "-122026-ZZZZZZ") {
		t.Fatalf("got %q", got)
	}
}

func TestFormatInvoiceNumberRandom(t *testing.T) {
	got := FormatInvoiceNumber("BTC-2026090001", time.Date(2026, 9, 10, 0, 0, 0, 0, time.UTC))
	if !strings.HasPrefix(got, "INV-BTC-2026090001-092026-") {
		t.Fatalf("got %q", got)
	}
	suf := strings.TrimPrefix(got, "INV-BTC-2026090001-092026-")
	if len(suf) != 6 {
